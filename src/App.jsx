...
const PodIcon = ({ ip, highlight, dead, small }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, opacity: dead ? 0.35 : 1, transition: "opacity 0.4s" }}>
      ...
      {ip && <span style={{ fontSize: 9, color: dead ? "#4b5563" : "#94a3b8", fontFamily: "monospace" }}>{dead ? "☠ crashed" : ip}</span>}
    </div>
  );
};
...
const Arrow = ({ label, color = "#0ea5e9", dashed = false, dim = false }) => (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2, opacity: dim ? 0.25 : 1, transition: "opacity 0.4s" }}>
...
// Шаг 1: нестабильный IP у бекенд-пода → фронтенд не может достучаться
// Шаг 2: несколько бекенд-подов → непонятно на какой балансировать
// Шаг 3: решение — frontend-pod → backend-svc → backend-pods

const svcSteps = [
  {
    title: "Проблема 1: нестабильные IP",
    subtitle: "Бекенд-под упал → перезапустился → получил новый IP. Фронтенд не знает куда идти.",
    deadBackendPod: "be-1",
    showService: false,
    arrowColor: "#ef4444",
    ...
    badgeColor: "#ef4444",
    badgeBg: "#2d1515",
    badgeText: "⚠ IP бекенда изменился после рестарта",
  },
  {
    title: "Проблема 2: балансировка нагрузки",
    subtitle: "Несколько бекенд-подов — на какой из них слать запросы?",
    deadBackendPod: null,
    showService: false,
    arrowColor: "#eab308",
    ...
    badgeColor: "#eab308",
    badgeBg: "#1a1a00",
    badgeText: "? на какой бекенд-под слать запрос?",
  },
  {
    title: "Решение: Kubernetes Service",
    subtitle: "frontend-pod обращается к backend-svc — стабильный IP и балансировка.",
    deadBackendPod: null,
    showService: true,
  },
];
...
function ServicesPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080c14", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "monospace", padding: 32, gap: 24 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", background: "#0d1117", border: "1px solid #1e293b", borderRadius: 20, padding: "32px 40px", gap: 0 }}>

        {/* Frontend pods — источник запроса, без проблем */}
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 9, color: "#64748b", letterSpacing: 2 }}>FRONTEND</span>
          <div style={{ display: "flex", gap: 14 }}>
            {fePods.map(p => <PodIcon key={p.id} ip={p.ip} highlight={cur.showService} />)}
          </div>
        </div>

        {/* Стрелка от фронтенда к бекенду */}
        {cur.showService ? (
          <>
            <div style={{ margin: "0 10px" }}><Arrow color="#0ea5e9" label="запрос" /></div>
            <SvcBox label="backend-svc" ip="10.96.20.20 (stable)" highlight />
            <div style={{ margin: "0 10px" }}><Arrow color="#0ea5e9" label="балансировка" /></div>
          </>
        ) : (
          <div style={{ margin: "0 18px" }}>
            <Arrow color={cur.arrowColor} dashed label={cur.arrowLabel} />
          </div>
        )}

        {/* Backend pods — здесь проблема */}
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 9, color: "#64748b", letterSpacing: 2 }}>BACKEND</span>
          <div style={{ display: "flex", gap: 14 }}>
            {bePods.map(p => <PodIcon key={p.id} ip={p.ip} highlight={cur.showService} dead={cur.deadBackendPod === p.id} />)}
          </div>
          {!cur.showService && (
            <div style={{ marginTop: 4, background: cur.badgeBg, border: `1px solid ${cur.badgeColor}`, borderRadius: 8, padding: "4px 12px", fontSize: 9, color: cur.badgeColor }}>
              {cur.badgeText}
            </div>
          )}
        </div>
      ...
      {/* DNS hint на последнем шаге */}
      {cur.showService && (
        <div style={{ background: "#0a1628", border: "1px dashed #6366f1", borderRadius: 12, padding: "10px 24px", display: "flex", gap: 12, alignItems: "center", animation: "fadeIn 0.5s ease" }}>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        ...
        <button onClick={() => setStep(s => Math.min(svcSteps.length - 1, s + 1))} disabled={step === svcSteps.length - 1}
          style={{ padding: "8px 20px", borderRadius: 8, border: "1px solid #334155", background: step === svcSteps.length - 1 ? "#0d1117" : "#1e293b", color: step === svcSteps.length - 1 ? "#475569" : "#e2e8f0", cursor: step === svcSteps.length - 1 ? "default" : "pointer", fontSize: 12 }}>
          Вперёд →
        </button>
      </div>
      ...
      <nav style={{
        ...
        {NAV.map(n => (
          <button key={n.id} onClick={() => setPage(n.id)} style={{
            ...
            background: "transparent", color: page === n.id ? "#e2e8f0" : "#64748b",
